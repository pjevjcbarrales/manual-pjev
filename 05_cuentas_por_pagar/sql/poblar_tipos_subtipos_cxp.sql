...
-- Esquema: cxp
-- Entidades: catalogo_grupos_tramite (Tipos) y catalogo_procedimientos_afectacion (Subtipos)
-- Autor: Equipo de Arquitectura SIAF-PJEV
-- Fecha: 2026-09-02
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS cxp.catalogo_grupos_tramite (
    ...
    usuario_c VARCHAR(50) NOT NULL DEFAULT 'SISTEMA',
...
-- Asegurar columnas y eliminar CHECK restrictivo previo de 1 y 2
...
-- Retirar restricción previa que obligaba cpa_tipo_pago IN (1, 2)
ALTER TABLE cxp.catalogo_procedimientos_afectacion 
    DROP CONSTRAINT IF EXISTS catalogo_procedimientos_afectacion_cpa_tipo_pago_check;

ALTER TABLE cxp.catalogo_procedimientos_afectacion 
    ADD CONSTRAINT catalogo_procedimientos_afectacion_cpa_tipo_pago_check 
    CHECK (cpa_tipo_pago IN (1, 2, 3, 4));

-- Poblar / Actualizar los 15 Subtipos / Procedimientos Canónicos
...
(
    ...
    'Users', 'Servicios Personales', 'EMPLEADO', 1, 1,
...
(
    ...
    'Building', 'Recursos Materiales', 'PROVEEDOR', 1, 2,
...
(
    ...
    'Zap', 'Recursos Materiales', 'PROVEEDOR', 1, 3,
    true, 'VALIDADO', 'No genera orden de compra; se liquida contra aviso-recibo o factura del proveedor paraestatal/concesionario.'
),
(
...
(
    ...
    'UserCheck', 'Servicios Personales', 'EMPLEADO', 1, 6,
...
(
    ...
    'FolderCheck', 'Gastos por Comprobar', 'UNIDAD_ADMINISTRATIVA', 3, 2,
...
(
    ...
    'RotateCcw', 'Fondo Rotatorio', 'UNIDAD_ADMINISTRATIVA', 3, 3,
...
(
    ...
    'Landmark', 'Pasivo Circulante 2.1.1.7', 'TERCERO_INST', 4, 1,
...
(
    ...
    'Scale', 'Operación de Balance', 'TERCERO_INST', 4, 3,
...
-- ----------------------------------------------------------------------------
-- Permitir que cxp.afectaciones_gasto acepte tipo_pago del 1 al 4
ALTER TABLE cxp.afectaciones_gasto 
    DROP CONSTRAINT IF EXISTS afectaciones_gasto_tipo_pago_check;

ALTER TABLE cxp.afectaciones_gasto 
    ADD CONSTRAINT afectaciones_gasto_tipo_pago_check 
    CHECK (tipo_pago IN (1, 2, 3, 4));

-- Sincronizar columna f_grupo_codigo con f_procedimiento_codigo si hubiera nulos
